...
ARA連携は実験的な実装です。基本的な音声取り込み、波形表示、解析、テンポ・再生同期には対応しています。
ただし、**プラグイン内で行った編集内容（ピッチ修正など）をプロジェクトへ保存（永続化）する機能は未実装**です。
プロジェクトを開き直すと、クリップから音声が再取り込みされ、プラグイン内の編集はリセットされます。
また、動作はプラグインによって異なります。
