...
配布アーカイブを任意の場所へ展開し、`KitsuneTone.exe` を実行してください。インストールは不要なポータブル版です。

VST3プラグインは、`C:\Program Files\Common Files\VST3` などの標準フォルダーから自動検出されます。

> [!NOTE]
> 狐ToneはASIOに対応していません。低遅延で使う場合は、設定画面の「Windows WASAPI」バックエンドで
...
トランスポートの録音ボタン（●）または「再生しながら録音」（▶●）で、オーディオ入力を録音できます。
録音停止後はWAVファイルとして保存され、新しいサンプルトラックへ自動的に配置されます。

- 入力デバイスは設定画面（オーディオ → PortAudio → Input device）で選択します
